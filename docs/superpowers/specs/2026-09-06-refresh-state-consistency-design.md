...
1. 模式切换开始后，所有此前启动的刷新结果都不能更新当前状态呈现。
...
- `Capture()` 返回当前版本；
- `BeginMutation()` 原子递增版本并返回新版本；
- `CanApply(capturedRevision, mutationInProgress)` 仅在没有进行中的模式切换且捕获版本仍是当前版本时返回 `true`。
...
模式事务在设置 `_modeSwitchInProgress = true` 的同一位置调用 `BeginMutation()`，确保所有经过统一模式事务入口的预设、自定义、自动化和恢复操作都会使在途刷新失效。

## 测试

- `PowerStateRevisionGateTests` 使用真实 gate 实例验证初始版本可应用、开始 mutation 会淘汰旧快照、新快照可应用，以及 mutation 标记会阻止应用。
- `RefreshInteractionPresentationTests` 增加源码路由契约，确保刷新捕获版本并在应用状态前检查 gate；已有 F5、只读、重复刷新测试继续保留。
